...
class GameOfLife:
    ...
    def draw_grid(self):
        ...
        self.canvas.create_rectangle(0, 0, self.width * self.cell_size, self.height * self.cell_size, fill="white", width=0)
        
        # Draw grid lines
        for i in range(0, self.width * self.cell_size, self.cell_size):
            self.canvas.create_line(i, 0, i, self.height * self.cell_size, fill="lightgray")
        for i in range(0, self.height * self.cell_size, self.cell_size):
            self.canvas.create_line(0, i, self.width * self.cell_size, i, fill="lightgray")
    ...
    def next_frame(self):
        ...
        # Efficient update of the canvas
        cells_to_remove = set(self.grid.keys()) - set(new_grid.keys())
        cells_to_add = set(new_grid.keys()) - set(self.grid.keys())
